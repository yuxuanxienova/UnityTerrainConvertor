...
This proposal has been implemented. This document exists for historical reference and may be outdated. For up-to-date documentation, see the UsdPhysics overview page.

Copyright © 2020, Apple, NVIDIA, Pixar Animation Studios, version 1.0
...
Default Values
Some problems came up while specifying this schema in connection with default values. First, there is a recent change to USD that eliminates the possibility of not creating attributes for schema APIs, which used to be a convenient way to denote a request to use a default value for the attribute. We now instead specify default values explicitly, typically sentinel values that lie outside of the range of legal values for a particular attribute. For example, if an attribute is normally required to be non-negative, we use 
 to request a certain default behavior. Sometimes the attribute can use the entire floating point range, in which case we reserve what is effectively +/- infinity at the edges of this range as sentinels. We will use the floating point 
 literal which USD supports in files and schemas to denote this. We document such default sentinel behavior on a case by case basis in the schema.
...
It is not possible to have nested bodies. PhysicsRigidBodyAPI applied to anything in the subtree under a node that already has a PhysicsRigidBodyAPI are ignored. An exception is if a prim has an resetXformStack op. In this case it ignores the inherited rigid body API. Assigning its own rigid body API can then be used to make it dynamic again.
...
Density is assumed to be 
 (approximately the density of water) for volume computation when no other density is specified locally, or in rel-ed materials either locally or higher up in the tree, and this value is converted into the collider’s native units prior to being used for mass computation.

Mass is assumed to be 
 in the mass units used when none is provided explicitly, and there are no collision volumes to derive from.
...
It is also possible to have PhysicsCollisionAPI on nodes that are not under a PhysicsRigidBodyAPI. These are treated as static colliders – shapes that are not moved by physics, but they can still collide with bodies, at which point they are interpreted as having zero velocity and infinite mass.
...
The subtypes PhysicsSphericalJoint, PhysicsRevoluteJoint and PhysicsPrismaticJoint both define a primary axis (Following the USD axis definition pattern established in e.g. GeomCapsule and GeomCylinder) and a top and bottom motion limit along it. PhysicsDistanceJoint defines a min and max distance between the attachment points. The PhysicsFixedJoint has no additional properties and simply locks all relative degrees of freedom.
